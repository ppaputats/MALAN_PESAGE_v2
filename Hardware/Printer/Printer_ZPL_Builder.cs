 ...
 *  Fichier     : Printer_ZPL_Builder.cs
 *  Auteur      : Paul Paput
 *  Création    : 16/02/26
 *  Description : Créateur du fichier ZPL
 *  Historique :
 *      - 16/02/26 : Création de la classe — Paul Paput
 *      - 18/02/26 : Mise à jour pour renforcer la robustesse de l'horodatage — Paul Paput
 *
 * ================================================================================================ */
using System;
...
{
    internal class Printer_ZPL_Builder
    {
        internal string Build_ZPL(int depLivre, string contact, string adresseLivr, string timestamp, int id_BL, int operateur, int idAR, int nbrColisTheo, int numero_colis, float poidsTheoNet, float poids, int id_pal)
        {
            Log_error log_Error = new Log_error();
            // Conversion des variables en string
            string depLivreStr = depLivre.ToString();
            string id_BLStr = id_BL.ToString();
            string operateurStr = operateur.ToString();
            string idARStr = idAR.ToString();
            string nbrColisTheoStr = nbrColisTheo.ToString();
            string numero_colisStr = numero_colis.ToString();
            string poidsTheoNetStr = poidsTheoNet.ToString("F2");
            string poidsStr = poids.ToString("F2");
            string id_palStr = id_pal.ToString();

            // Construction du ZPL avec une chaîne verbatim interpolée
            string zpl = $@"
^XA
...
^CF0,150
^FO30,30^FD{depLivre}^FS

^CF0,35
^FO250,30^FD{contact}^FS
^FO250,75^FD{adresseLivr}^FS
...
^FO30,340^FDID AR : {idAR}^FS
^FO30,380^FDNb colis théorique : {nbrColisTheo}^FS
^FO30,420^FDNuméro colis/palette : {numero_colis}^FS

^FO30,480^FDPoids théorique net : {poidsTheoNet} kg^FS
^FO30,520^FDPoids mesuré : {poids} kg^FS

^FO30,560^FDPalette : {id_pal}^FS
